...
func (m *Go01) Publish(ctx context.Context, source *Directory) (string, error) {
	...
	prodImage := dag.Container().
		...
		//WithFile("/go/bin/test.json", source.File("src/test.json")).
		WithFile("/go/bin/1.json", builder.File("/src/src/1.json")).
		WithFile("/go/bin/2.json", builder.File("/src/src/2.json")).
		WithWorkdir("/go/bin").
	...
	address, err := prodImage.Publish(ctx, fmt.Sprintf("filstimatias/dagger-test-aux:%.0f", math.Floor(rand.Float64()*100)))
	if err != nil {
		return "", err
	}
	return address, nil
